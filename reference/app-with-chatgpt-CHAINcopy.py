...
flavor_options = {
    "Summary(General)": "a helpful assistant data analyst who uses ngrams from product reviews to summarize that customers do and do not like.",
...
@st.cache_resource
def load_chatgpt(temp,flavor):
    # Clear message history and specify flavor
    st.session_state.session_messages = [
    SystemMessage(content=flavor_options[flavor])
    ]
    cjat = ChatOpenAI(temperature=temp)
    return 
...
flavor = col1.selectbox("Which type of chatbot?", key='no_reset',options=list(flavor_options.keys()), index=0,)
temp = col2.slider("Select model temperature:",min_value=0.0, max_value=2.0, value=0.1)
reset_chat = col3.button("Reset chatgpt?")
...
#     flavor_text = f"The following is a conversation between a human and an AI analyst. The AI is {flavor}."
